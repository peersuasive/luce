...
//==============================================================================
struct MP3FrameInfinite
{
    MP3FrameInfinite()
    {
        zeromem (this, sizeof (MP3FrameInfinite));
        single = -1;
    }
    ...
    void decodeHeader (const uint32 header)
    {
        ...
        if (bitrateIndex == 0)
        {
            DBG("!! wrong bitrateIndex !!");
            DBG(" -> layer: "+String(layer));
            DBG(" -> mode: "+String(mode));
            DBG(" -> bitrateIndex: "+String(bitrateIndex));
            DBG(" -> numChannels: "+String(numChannels));
        ...
        else if (bitrateIndex<0 || bitrateIndex>=15)
            std::cout << "OMG! how can this be this value ????"
...
//==============================================================================
struct MP3StreamInfinite
{
    MP3StreamInfinite (InputStream& source)
        : stream (source, 8192),
          numFrames (0), currentFrameIndex (0), vbrHeaderFound (false)
    {
        reset();
        isInfinite = stream.getTotalLength()<0;
    }

    int decodeNextBlock (float* const out0, float* const out1, int& done)
    {
        bool seeking = out0==nullptr;
        if(seeking) {
            std::cout << "header parsed ? " << headerParsed << std::endl;
        }
        if (! headerParsed)
        {
            ...
            //if(frame.frameSize==0 && isInfinite) {
            //    DBG("FRAME < 0");
            //    return 0;
            //}
            headerParsed = true;
        ...
        if (isFreeFormat)
        {
            ...
            {
                const int nextFrameOffset = scanForNextFrameHeader (!isInfinite); //true);
    ...
    bool seek (int frameIndex)
    {
        ...
        std::cout << "seek: frameIndex: " << frameIndex << ", size: " << frameStreamPositions.size()
            << ", stored interval: " << storedStartPosInterval
            << ", true ? " << (frameIndex>=frameStreamPositions.size() * storedStartPosInterval)
            << std::endl;
        while (frameIndex >= frameStreamPositions.size() * storedStartPosInterval)
        {
            ...
            if (result > 0 || (isInfinite && result==0))
                break;
...
private:
    ...
    static bool isValidHeader (const uint32 header, const int oldLayer) noexcept
    {
                ...
                && (header & 3) != 2)
        {
            ...
            int mode                = (header >> 6) & 3;
            int numChannels         = (mode == 3) ? 1 : 2;

            if(bitrateIndex==0 || bitrateIndex==15) {
                DBG("*** discarding header with wrong bitrateIndex");
                DBG("checking header:");
                DBG(" layer: "+String(layer));
                DBG(" mode: "+String(mode));
                DBG(" bitrateIndex: "+String(bitrateIndex));
            ...
            return bitrateIndex != 0;
        }
        /*
        else {
            DBG("** discarding wrong header **");
        }
        */
        return false;
        /*
    ...
    int scanForNextFrameHeader (const bool checkTypeAgainstLastFrame) noexcept
    {
        ...
        for (int i=0;;++i)
        {
            int64 curPos = stream.getPosition();
            if (stream.isExhausted() || stream.getPosition() > oldPos + 32768)
            {
                DBG("exhausted stream ! (" + String( stream.isExhausted() ) + String(")") );
                std::cout << "length: " << stream.getTotalLength() << ", pos: " << stream.getPosition() << std::endl;
                offset = -1;
                break;
            }
            /*
            else if (i>0 && stream.getPosition() == prevPos) {
                if( ++prevCount > 10 ) {
                    DBG("stream position unchanged -- silently exhausted ?");
                    std::cout << "prevPos: " << prevPos << ", cur pos: " << stream.getPosition() << std::endl;
                    offset = -1;
                    prevCount = 0;
                    //return offset;
                    break;
            ...
            */
            ...
            header = (header << 8) | (uint8) c;
            if(curPos==stream.getPosition()) {
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MP3StreamInfinite)
};

//==============================================================================
static const char* const mp3FormatName = "MP3 (infinite) file";

//==============================================================================
class MP3ReaderInfinite : public AudioFormatReader
{
public:
    MP3ReaderInfinite (InputStream* const in)
                      ...
                      int64 startSampleInFile, int numSamples) override
    {
        ...
        {
            std::cout << "currentPosition expired: " << currentPosition << ", " << startSampleInFile << std::endl;
            if (! stream.seek ((int) (startSampleInFile / 1152 - 1)))
            {
                bool pos_set = stream.stream.setPosition(startSampleInFile);
                std::cout << "position set ? " << pos_set << std::endl;
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MP3ReaderInfinite)
};

}

//==============================================================================
MP3AudioFormatInfinite::MP3AudioFormatInfinite()  : AudioFormat (MP3DecoderInfinite::mp3FormatName, ".mp3") {}
MP3AudioFormatInfinite::~MP3AudioFormatInfinite() {}

Array<int> MP3AudioFormatInfinite::getPossibleSampleRates() { return Array<int>(); }
Array<int> MP3AudioFormatInfinite::getPossibleBitDepths()   { return Array<int>(); }
bool MP3AudioFormatInfinite::canDoStereo()                  { return true; }
bool MP3AudioFormatInfinite::canDoMono()                    { return true; }
bool MP3AudioFormatInfinite::isCompressed()                 { return true; }
StringArray MP3AudioFormatInfinite::getQualityOptions()     { return StringArray(); }

AudioFormatReader* MP3AudioFormatInfinite::createReaderFor (InputStream* sourceStream, const bool deleteStreamIfOpeningFails)
{
    ScopedPointer<MP3DecoderInfinite::MP3ReaderInfinite> r (new MP3DecoderInfinite::MP3ReaderInfinite (sourceStream));

    if (r->lengthInSamples<0 || r->lengthInSamples > 0)
        return r.release();
...
AudioFormatWriter* MP3AudioFormatInfinite::createWriterFor (OutputStream*, double /*sampleRateToUse*/,
