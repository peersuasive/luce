...
AudioTransportInfiniteSource::AudioTransportInfiniteSource()
...
void AudioTransportInfiniteSource::setSource (PositionableAudioSource* const newSource,
                                      int readAheadSize, TimeSliceThread* readAheadThread,
                                      double sourceSampleRateToCorrectFor, int maxNumChannels)
{
    if (source == newSource && source == nullptr) return;
    AudioTransportSource::setSource(newSource, readAheadSize, readAheadThread, sourceSampleRateToCorrectFor, maxNumChannels);
    source = newSource;
...
void AudioTransportInfiniteSource::setGain (const float newGain) noexcept
{
    gain = newGain;
...
void AudioTransportInfiniteSource::releaseResources()
{
    AudioTransportSource::releaseResources();
    source = nullptr;
}

void AudioTransportInfiniteSource::getNextAudioBlock (const AudioSourceChannelInfo& info)
{
    const ScopedLock sl (callbackLock);
    int playing = AudioTransportSource::isPlaying();

    if (source != nullptr && playing) {
        ...
        int total_length = source->getTotalLength()+1;
        if(total_length>0 && source->getNextReadPosition() > total_length && ! source->isLooping())
        {
            playing = false;
        }
        for (int i = info.buffer->getNumChannels(); --i >= 0;)
        ...
        if(!playing) AudioTransportSource::stop();
    }
    else {
...
/*
void AudioTransportInfiniteSource::start()
{
...
void AudioTransportInfiniteSource::stop()
{
    if (playing)
    {
        {
...
void AudioTransportInfiniteSource::setPosition (double newPosition)
{
    if (sampleRate > 0.0)
        setNextReadPosition ((int64) (newPosition * sampleRate));
}

double AudioTransportInfiniteSource::getCurrentPosition() const
{
    if (sampleRate > 0.0)
...
double AudioTransportInfiniteSource::getLengthInSeconds() const
{
    if (sampleRate > 0.0)
...
void AudioTransportInfiniteSource::setNextReadPosition (int64 newPosition)
{
...
int64 AudioTransportInfiniteSource::getNextReadPosition() const
{
    if (positionableSource != nullptr)
    {
...
int64 AudioTransportInfiniteSource::getTotalLength() const
{
    const ScopedLock sl (callbackLock);
...
bool AudioTransportInfiniteSource::isLooping() const
{
    const ScopedLock sl (callbackLock);
...
void AudioTransportInfiniteSource::prepareToPlay (int samplesPerBlockExpected, double newSampleRate)
{
    const ScopedLock sl (callbackLock);
...
void AudioTransportInfiniteSource::releaseMasterResources()
{
    const ScopedLock sl (callbackLock);
